...
vim.api.nvim_create_autocmd('VimEnter', {
  ...
  callback = function()
    PackUtils.load({ name = 'nvim-ufo', deps = { 'promise-async' } }, function()
      ...
      -- ufo 会在整个 buffer 重载时清空折叠，但不会主动重新请求 provider。
      vim.api.nvim_create_autocmd({ 'BufReadPost', 'FileChangedShellPost' }, {
        ...
        callback = function(args)
          local buf = args.buf
          if not ufo.hasAttached(buf) then return end
          vim.schedule(function()
            if vim.api.nvim_buf_is_valid(buf) and ufo.hasAttached(buf) then ufo.enableFold(buf) end
          end)
        end,
      })
