...
map('n', 'S', ':IntelliSave<cr>', { desc = 'save file' })
...
-- space 行首行尾跳转
map('n', '<space>', ':IntelliJump<cr>', { desc = 'jump line start to end' })
map('n', '0', '%', {})
map('v', '0', '%', {})
...
--[[ 浮动终端 ]]
map('n', '<F5>', ':lua require("configs.terminal").runFile()<cr>', { desc = 'run file' })
map('i', '<F5>', '<esc>:lua require("configs.terminal").runFile()<cr>', { desc = 'run file' })
