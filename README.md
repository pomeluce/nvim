...
| 快捷键       | 功能                              |
| ------------ | --------------------------------- |
| `<c-t>`      | 打开、聚焦终端或等待一次 tab 操作 |
| `<c-o>`      | 打开或隐藏独立的 Codex Agent 终端 |
| `<leader>rf` | 运行当前文件                      |

浮动终端支持多 tab，并使用类似 tmux 的一次性前导键，避免占用 shell 和终端模拟器常用的 `Alt` 组合键：终端隐藏时按 `<C-t>` 直接打开；终端可见但未聚焦时按 `<C-t>` 会先聚焦；已聚焦时按 `<C-t>` 后再输入一个操作键。操作执行后立即结束，不需要额外按 `<Esc>` 退出，也不会安装临时的全局单键映射。
