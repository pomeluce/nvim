...
推荐方式：在 `flake.nix` 中引入 `homeManagerModules.default`，所有依赖自动注入，settings 通过 Nix options 配置：
      ...
      modules = [
        nvim.homeManagerModules.default
        {
          programs.akirnvim = {
            ...
            # settings.toml 配置（均为可选）
            settings = {
              session.projects = [ "~/devspace/code/my-project" ];
              lsp.jdtls.runtimes = [
                { name = "JavaSE-21"; path = "/usr/lib/jvm/java-21-openjdk"; default = true; }
              ];
              header.tmpl.python = "#!/usr/bin/env python3\n# author: {USER}";
...
如果不使用 flake，可在 `configuration.nix` 或 home-manager 中手动声明 `programs.neovim.extraPackages`：

<details>
<summary>展开查看完整包列表</summary>
...
以下环境变量被配置引用。**flake 用户**由 `programs.akirnvim.env` 自动推导（无需手动设置）；**手动安装的 NixOS 用户**在 `home.sessionVariables` 中设置；**非 NixOS 用户**在 shell 配置（`.zshrc` / `.bashrc`）中设置。
...
使用 flake 模块时，环境变量由 `programs.akirnvim` 自动推导，无需手动设置：

```nix
# 通过 flake 自动注入（默认值）：
# JAVA_LOMBOK     → ${pkgs.lombok}/share/java/lombok.jar
# VSC_JAVA_DEBUG  → vscode-extensions 路径
# VSC_JAVA_TEST   → vscode-extensions 路径
# VSC_FIREFOX_DEBUG → vscode-extensions 路径

# 如需覆盖：
programs.akirnvim.env.JAVA_LOMBOK = "/custom/path/lombok.jar";
```
...
| 占位符        | 说明                                                      |
| ------------- | --------------------------------------------------------- |
| `{USER}`      | 作者名, 优先级: `header.env.USER` > `$USER` / `$USERNAME` |
...
**自定义模板：** 在 `settings.toml` 中添加 `[header.tmpl]` 节覆盖任意语言模板：
...
**占位符变量覆盖 (`header.env`)：** 模板里的 `{USER}` 默认读取系统用户名。若需指定固定作者名（而非当前系统用户），在 `[header.env]` 中覆盖即可，模板照常使用 `{USER}` 占位符，无需逐个硬编码：
...
`header.env` 是通用的占位符变量表，优先级高于内置来源（环境变量等）。除覆盖 `{USER}` 等内置占位符外，也可扩展自定义占位符 —— 在模板里写 `{COMPANY}`，再在 `[header.env]` 中定义 `COMPANY = "..."` 即可自动替换。
